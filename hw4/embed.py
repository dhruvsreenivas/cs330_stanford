...
import collections
import numpy as np
from rlax import epsilon_softmax
import torch
...
class TransitionEmbedder(Embedder):
    ...
    def forward(self, experiences):
        ...
        transition_embeds = self._transition_embedder(
...
class EncoderDecoder(Embedder, relabel.RewardLabeler):
    ...
    def _compute_losses(
        ...
        # ******************* YOUR CODE HERE *********************
        # ********************************************************
        episode_len_plus_one = all_decoder_contexts.shape[1]
        # (batch_size, episode_len + 1, embed_dim)
        zs = torch.stack(
            [id_contexts] * episode_len_plus_one, dim=1)

        # (batch_size, episode_len + 1)
        decoder_context_loss = torch.norm(
            all_decoder_contexts - zs, p=2, dim=-1).square()
    ...
    def label_rewards(self, trajectories):
        ...
        rewards = torch.stack([distances[:, i] - distances[:, i-1]
                              for i in range(1, episode_len_plus_one)], dim=1)
