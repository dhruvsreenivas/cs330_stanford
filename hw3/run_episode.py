...
def run_episode(
    ...
    for _ in range(steps_per_episode):
        ...
        inp = torch.cat(
            [torch.from_numpy(state), torch.from_numpy(goal_state)], dim=0).float()
        action = q_net(inp).argmax().item()
        next_state, reward, done, info = env.step(action)
        tup = (state, action, reward, next_state, goal_state)
        episode_experience.append(tup)
