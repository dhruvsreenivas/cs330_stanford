...
def update_replay_buffer(
    ...
    for timestep in range(len(episode_experience)):
        ...
        if her_type == HERType.FINAL:
            ...
            # add to buffer
            _, _, _, fg, _ = episode_experience[-1]
            fg = fg.copy()
            s = state.copy()
            ns = next_state.copy()
            r = env_reward_function(s, fg)
            ...
            for _ in range(num_relabeled):
                if timestep == len(episode_experience) - 1:
                    rand_idx = len(episode_experience) - 1
                else:
                    rand_idx = np.random.randint(
                        timestep + 1, len(episode_experience))
                _, _, _, rand_g, _ = episode_experience[rand_idx]
                rand_g = rand_g.copy()
                s = state.copy()
                ns = next_state.copy()
                r = env_reward_function(s, rand_g)
            ...
            # add to replay buffer
            for _ in range(num_relabeled):
                rand_idx = np.random.randint(len(episode_experience))
                _, _, _, rand_g, _ = episode_experience[rand_idx]
                rand_g = rand_g.copy()
                s = state.copy()
                ns = next_state.copy()
